...
int main()
{
  ...
  printf("The second hostaddress is: %s\n",
  ...
  for (;;)
  {
    ...
    if (read(sfd, ch, 1) != 1)      // Read from socket
    {    
        OOPS("Read has failed!");
    }
    pid_t res = fork();             // fork
    if (res == 0)                    // child
    {
        ...
        switch (ch[0]) 
        {
            ...
            case '3':
                ...
                break;
            default:
                execlp("echo", "echo", "That option doesn't work!", NULL);
        }
        OOPS("Exec has failed!");
    }
